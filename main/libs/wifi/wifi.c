...
void wifi_init_sta(unsigned char *wifi_ssid, unsigned char *wifi_pass)
{
    ...
    unsigned_char_buf_write(wifi_config.sta.ssid, wifi_ssid);
    unsigned_char_buf_write(wifi_config.sta.password, wifi_pass);
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "connected to ap SSID:%s password:%s", DEFAULT_WIFI_SSID,
                 DEFAULT_WIFI_PASS);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(TAG, "Failed to connect to SSID:%s, password:%s",
                 DEFAULT_WIFI_SSID, DEFAULT_WIFI_PASS);
    } else {
...
{
    unsigned char *default_ssid = (uint8_t *)DEFAULT_WIFI_SSID;
    unsigned char *default_pass = (uint8_t *)DEFAULT_WIFI_PASS;
    wifi_init_sta(default_ssid, default_pass);
}
